...
def train(trainloader, milnet: mil.MILNet, criterion, optimizer):
    ...
    for i, (bag, bag_label) in enumerate(trainloader):
        ...
        bag_predictions = torch.stack(bag_predictions)
...
def test(config, testloader, milnet, criterion, thresholds=None, return_predictions=False, validation=False):
    ...
    with torch.no_grad():
        for i, (bag, bag_label) in enumerate(testloader):
            ...
            if config.average:
                test_predictions.extend([(torch.sigmoid(max_prediction)+torch.sigmoid(bag_predictions)).squeeze().cpu().numpy()])
            else: test_predictions.extend([torch.sigmoid(bag_predictions).squeeze().cpu().numpy()])
...
class OG_DSMIL():
    ...
    def run(self, trainset:DiffInfiniteDataset, testset:DiffInfiniteDataset, feature_extractor):
        ...
        for iteration in range(5):
            ...
            test_loss_bag, avg_score, aucs, thresholds_optimal = test(self.config, testloader, best_model, criterion)
